...
def spawn_armando(robot_id, x_pose, y_pose, z_pose):
    ...
    Args:
        robot_id: ID del robot (1-2)
        x_pose, y_pose, z_pose: Posizione nel mondo
    ...
    pkg_share_path = get_package_share_directory('final_proj')
    
    yaml_file = f'armando_controllers_{robot_id}.yaml'
    yaml_absolute_path = os.path.join(pkg_share_path, 'config', yaml_file)
    
    urdf_absolute_path = os.path.join(pkg_share_path, 'urdf', 'arm.urdf.xacro')
    
    mesh_dir = os.path.join(pkg_share_path, 'meshes')
...
def generate_launch_description():
    """
    Spawna 2 Armando robots nel mondo workshop
    ...
    # Spawn tutti e 2 Armando con delay tra robot
    for robot_id in range(1, 3):
        ...
        if robot_id == 1:
            ...
            # Altro robot: delay di 5 secondi tra ciascuno
